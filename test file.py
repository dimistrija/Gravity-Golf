...
def check_object_collision(ball_pos, obj_pos):
    return (obj_pos[0] - OBJECT_SIZE / 2 <= ball_pos[0] <= obj_pos[0] + OBJECT_SIZE / 2 and
            obj_pos[1] - OBJECT_SIZE / 2 <= ball_pos[1] <= obj_pos[1] + OBJECT_SIZE / 2)
...
while running:
    ...
    if display_collected and current_time - display_start_time < 3000:  # Display for 3 seconds
        screen.blit(saved_img, (
        SCREEN_WIDTH // 2 - saved_img.get_width() // 2, SCREEN_HEIGHT // 2 - saved_img.get_height() // 2))
    else:
        display_collected = False
